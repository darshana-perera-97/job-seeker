function StatsCard({ title, value, icon: Icon, trend, trendUp }) {
  ...
  const hasTrend = typeof trendText === 'string' && trendText.trim().length > 0;
  const trendColor = trendUp ? '#627B4A' : '#D4183D';

  return (
    <div className="overflow-hidden rounded-xl shadow-sm hover:shadow-md transition-shadow dark:bg-[#1A1F2E] bg-white dark:border dark:border-[rgba(108,166,205,0.2)]">
